...
import { Filters } from "context/HomePageProvider/HomePageProvider.types";
  ...
  return (
    <div className='max-w-7xl mb-8 flex justify-center mt-6 md:mt-0 md:justify-start gap-x-4'>
      <div className='relative'>
        ...
        <div
          className={`transform transition-all absolute ${
            states.type.isFilterModalOpen ? "scale-100" : "scale-0"
        ...
        >
          {filterFields.map(({ value, label }, index) => (
            <div key={index}>
              <button
                className={`${
                  value === states.type.value ? "text-brandsPrimary" : ""
      ...
      <div className='relative'>
        ...
        <div
          className={`transform transition-all absolute ${
            states.date.isFilterModalOpen ? "scale-100" : "scale-0"
          } w-32 py-1 border border-brandsPrimary z-20 bg-white rounded-md
        mt-2 px-2`}
        >
          {filterDates.map((field, index) => (
            <div key={index}>
              <button
                className={`${
                  field === states.date.value ? "text-brandsPrimary" : ""
                } hover:text-brandsPrimary`}
                onClick={() => onChange("date", field)}
              >
                {field}
              </button>
            </div>
